    ...
    public void processData(byte b) {
        if (doCollectData) {
            ...
            if (b == preamble[preambleCounter]) {
                ...
                ++preambleCounter;
                LogHelper.simpleLog(this.getClass().toString(), "Correct preamble!");
            } else {
                // if the data byte is not a preamble sequence byte then reset the preamble counter
                preambleCounter = 0;
                LogHelper.simpleLog(this.getClass().toString(), "False preamble! " + b + " with " + preamble[preambleCounter]);
            }
            ...
            if (preambleCounter == SerializationTypes.SIZEOF_INTEGER) {
                ...
                LogHelper.simpleLog(this.getClass().toString(), "Preamble not received!");
            }
        }
    }
    ...
    protected void collectData(byte b) {
        ...
        if (postambleCounter == SerializationTypes.SIZEOF_INTEGER) {
            ...
            if (SerializationTypes.bytesToInteger(checksumBytes) == bufferCounter + 1) {
                ...
                LogHelper.simpleLog(this.getClass().toString(), "SerializationReader: Checksum failed!");
            }
            ...
            LogHelper.simpleLog(this.getClass().toString(), "Packet corrupted!");
        }
    }
}
